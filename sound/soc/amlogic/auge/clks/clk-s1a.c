...
#undef pr_fmt
#define pr_fmt(fmt) "s1a_audio_clocks: " fmt
...
static const char *const mclk_parent_names[] = {
	"hifi1_pll", "null", "null", "null", "hifi_pll",
...
static struct clk_gate *s1a_audio_clk_gates[] = {
	&audio_ddr_arb,
	&audio_pdm,
	&audio_tdmina,
	&audio_tdminb,
...
static struct clk_hw *s1a_audio_clk_hws[] = {
...
static int s1a_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(s1a_audio_clk_gates) != MCLK_BASE) {
	...
	for (clkid = 0; clkid < MCLK_BASE; clkid++) {
		unsigned long offset =
			(unsigned long)s1a_audio_clk_gates[clkid]->reg;
		s1a_audio_clk_gates[clkid]->reg =
			(void __iomem *)((unsigned long)iobase + offset);
		clks[clkid] = clk_register(NULL, s1a_audio_clk_hws[clkid]);
...
static int s1a_clks_init(struct clk **clks, void __iomem *iobase)
{
...
struct audio_clk_init s1a_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = s1a_clk_gates_init,
	.clks      = s1a_clks_init,
};
