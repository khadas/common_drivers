...
static void meson_osd_get_primary_plane_index(struct meson_drm *priv)
{
	...
	for (i = 0; i < pipeline->num_postblend; i++) {
		DRM_INFO("primary_plane_index-%d, osd_index-%d\n",
	...
	struct drm_plane_state *new_state)
{
	...
	ret = meson_plane_atomic_check(plane, new_state->state);
	if (ret != 0) {
	...
	struct drm_plane_state *new_state)
{
	...
	ret = meson_video_plane_atomic_check(plane, new_state->state);
	if (ret) {
	...
	struct drm_plane_state *old_state)
{
	...
	struct meson_vpu_pipeline *pipeline = amcrtc->pipeline;
	int vrefresh = drm_mode_vrefresh(&plane->crtc->mode);
...
static struct am_osd_plane *am_osd_plane_create(struct meson_drm *priv, int i)
{
	...
	if (type == DRM_PLANE_TYPE_PRIMARY)
		possible_crtcs |= 1 << j;
	DRM_INFO("possible_crtcs-%d type-%d\n", possible_crtcs, type);
