...
OPRION_PARAM="$*"
if [[ "${OPRION_PARAM}" =~ "--c3_debug" ]]; then
	echo "make meson64_a32_C3_mini_defconfig & C3_debug_defconfig"
	DEBUG=1
	OPRION_PARAM=${OPRION_PARAM//--c3_debug/}
...
source ${ROOT_DIR}/common/common_drivers/scripts/amlogic/mk_smarthome_common.sh ${OPRION_PARAM}

[[ "${DEBUG}" == "1" ]] && rm -f ${CONFIG_DIR}/${DEFCONFIG}*
