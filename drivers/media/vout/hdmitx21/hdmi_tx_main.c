...
static int calc_vinfo_from_hdmi_timing(const struct hdmi_timing *timing, struct vinfo_s *tx_vinfo)
{
	...
	tx_vinfo->viu_mux |= hdev->tx_hw.enc_idx << 4;
			   ...
			   const char *buf, size_t count)
{
	...
	hdmitx_hw_cntl_ddc(&hdev->tx_hw.base, DDC_SCDC_DIV40_SCRAMB, buf[0] - '0');
...
static int amhdmitx_get_dt_info(struct platform_device *pdev, struct hdmitx_dev *hdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = of_property_read_u32(pdev->dev.of_node,
					   "enc_idx", &val);
		hdev->tx_hw.enc_idx = 0; /* default 0 */
		if (!ret) {
			if (val == 2)
				hdev->tx_hw.enc_idx = 2;
		}
