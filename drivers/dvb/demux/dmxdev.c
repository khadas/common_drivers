...
static long dvb_demux_ioctl(struct file *file, unsigned int cmd,
			    unsigned long arg)
{
	return usercopy(file, cmd, arg, dvb_demux_do_ioctl);
}
...
static long dvb_dvr_ioctl(struct file *file,
			 unsigned int cmd, unsigned long arg)
{
	return usercopy(file, cmd, arg, dvb_dvr_do_ioctl);
}
...
int amlogic_dvb_dmxdev_init(struct dmxdev *dmxdev, struct dvb_adapter *dvb_adapter)
{
	int i;
...
EXPORT_SYMBOL(amlogic_dvb_dmxdev_init);

void amlogic_dvb_dmxdev_release(struct dmxdev *dmxdev)
{
	dmxdev->exit = 1;
...
EXPORT_SYMBOL(amlogic_dvb_dmxdev_release);
