...
static int loopback_trigger(struct snd_soc_component *component,
			    struct snd_pcm_substream *ss,
			    int cmd)
{
	return 0;
...
static int loopback_pcm_new(struct snd_soc_component *component,
...
static void loopback_pcm_free(struct snd_soc_component *component, struct snd_pcm *pcm)
{
	struct snd_pcm_substream *ss;
...
static int datain_pdm_startup(struct loopback *p_loopback)
{
	...
	if (p_loopback->syssrc_clk_rate) {
		ret = clk_set_rate(p_loopback->pdm_sysclk_srcpll,
				   p_loopback->syssrc_clk_rate);
		if (ret) {
			pr_err("Can't set loopback pdm_sysclk_srcpll: %d\n", ret);
			return ret;
		}
	}
...
static int tdminlb_startup(struct loopback *p_loopback)
{
	...
	if (p_loopback->syssrc_clk_rate) {
		ret = clk_set_rate(p_loopback->tdminlb_mpll,
				   p_loopback->syssrc_clk_rate);
		if (ret) {
			pr_err("Can't set loopback tdminlb_mpll: %d\n", ret);
			goto err;
		}
	}
			     ...
			     int rate, bool enable)
{
	...
	if (p_loopback->syssrc_clk_rate)
		...
		ret = clk_set_rate(p_loopback->tdminlb_mpll, mpll_freq);
	if (ret) {
		pr_err("Can't set loopback tdminlb_mpll: %d\n", ret);
		return;
	}

	ret = clk_set_rate(p_loopback->tdminlb_mclk, mclk_freq);
	if (ret) {
		pr_err("Can't set loopback tdminlb_mclk: %d\n", ret);
		return;
	}
				...
				struct snd_soc_dai *dai)
{
	...
	if (ss->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		case DATAIN_TDMB:
		case DATAIN_TDMC:
			aml_tdmin_set_src(p_loopback->original_src);
			break;
		case DATAIN_SPDIF:
			break;
		case DATAIN_PDM:
	...
	static void
loopback_origin_src_trigger(struct loopback *p_loopback,
			    int stream,
			    bool enable)
{
	switch (p_loopback->datain_src) {
	...
	static void
loopback_origin_src_fifo_reset(struct loopback *p_loopback,
			       int stream)
{
	switch (p_loopback->datain_src) {
	case DATAIN_TDMA:
				...
				struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (ss->stream == SNDRV_PCM_STREAM_CAPTURE) {
			...
			loopback_origin_src_fifo_reset(p_loopback, ss->stream);
			...
			loopback_origin_src_trigger
				(p_loopback, ss->stream, true);
		}
		break;
...
static void datain_pdm_set_clk(struct loopback *p_loopback)
{
...
#else
	ret = clk_set_rate(p_loopback->pdm_sysclk, 133333351);
	if (ret) {
		pr_err("Can't set loopback pdm sys clock: %d\n", ret);
		return;
	}

	if (pdm_dclk_srcpll_freq == 0) {
		ret = clk_set_rate(p_loopback->pdm_dclk_srcpll, 24576000);
		if (ret) {
			pr_err("Can't set loopback pdm_dclk_srcpll: %d\n", ret);
			return;
		}
	} else {
	...
	ret = clk_set_rate(p_loopback->pdm_dclk,
			   pdm_dclkidx2rate(p_loopback->dclk_idx));
	if (ret) {
		pr_err("Can't set loopback pdm_dclk: %d\n", ret);
		return;
	}
...
static void datalb_tdminlb_set_clk(struct loopback *p_loopback)
{
	...
	if (p_loopback->syssrc_clk_rate)
		...
		ret = clk_set_rate(p_loopback->tdminlb_mpll, mpll_freq);
	if (ret) {
		pr_err("Can't set loopback syssrc/mpll clock: %d\n", ret);
		return;
	}

	ret = clk_set_rate(p_loopback->tdminlb_mclk, p_loopback->sysclk_freq);
	if (ret) {
		pr_err("Can't set loopback sys clock: %d\n", ret);
		return;
	}
			 ...
			 struct snd_soc_dai *dai)
{
	...
	aml_tdm_hw_setting_free(p_loopback->original_src, ss->stream);
				    ...
				    int mute, int stream)
{
	...
	if (p_loopback->datain_chnum > 0) {
		...
		case DATAIN_TDMB:
		case DATAIN_TDMC:
			tdm_mute_capture(p_loopback->original_src, mute);
			break;
		case DATAIN_SPDIF:
			break;
		case DATAIN_PDM:
...
SOC_ENUM_SINGLE(EE_AUDIO_LB_A_CTRL0, 0, ARRAY_SIZE(datain_src_texts),
		datain_src_texts);
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	tmp = p_loopback->datain_src;
	p_loopback->datain_src = ucontrol->value.enumerated.item[0];

	if (datain_parse_of(p_loopback->dev->of_node, p_loopback)) {
		p_loopback->datain_src = tmp;
		return -EINVAL;
	}
...
static const struct snd_soc_component_driver loopback_component = {
	.name         = DRV_NAME,
	.pcm_construct  = loopback_pcm_new,
	.pcm_destruct = loopback_pcm_free,
	.open      = loopback_open,
			       ...
			       struct loopback *p_loopback)
{
	...
	ret = of_property_read_u32(dev->of_node, "sys-src-clk-freq",
				   &p_loopback->syssrc_clk_rate);
	if (ret < 0)
...
static int loopback_platform_probe(struct platform_device *pdev)
{
	...
	np_src = of_parse_phandle(node, "original-src", 0);
	if (np_src) {
		...
		if (dev_src)
			p_loopback->original_src =
				platform_get_drvdata(dev_src);
	} else {
		pr_warn("%s(), warn: no node-%s\n",
			__func__, "original-src");
	}
