...
static int bl_pwm_out_level_check(struct bl_pwm_config_s *bl_pwm)
{
	...
	case BL_PWM_POSITIVE:
		if (bl_pwm->pwm_duty == 0)
			out_level = 0;
		else if (bl_pwm->pwm_duty == bl_pwm->pwm_duty_range)
			out_level = 1;
		else
			out_level = 0xff;
		break;
...
void bl_pwm_mapping_init(struct aml_bl_drv_s *bdrv)
{
	...
	case BL_CTRL_PWM:
		...
		if (bl_pwm) {
			bl_pwm->pwm_mapping[0] = bl_pwm->bl_level_min;
			bl_pwm->pwm_mapping[1] = bl_pwm->bl_level_min +
				(bl_pwm->bl_level_max - bl_pwm->bl_level_min + 2) / 4;
			bl_pwm->pwm_mapping[2] = bl_pwm->bl_level_min +
				2 * ((bl_pwm->bl_level_max - bl_pwm->bl_level_min + 2) / 4);
			bl_pwm->pwm_mapping[3] = bl_pwm->bl_level_min +
				3 * ((bl_pwm->bl_level_max - bl_pwm->bl_level_min + 2) / 4);
			bl_pwm->pwm_mapping[4] = bl_pwm->bl_level_max;
...
static unsigned int bl_pwm_set_mapping(struct bl_pwm_config_s *bl_pwm, unsigned int level)
{
	...
	if (p0 < bl_pwm->bl_level_min || p4 > bl_pwm->bl_level_max)
		BLERR("pwm mapping curve is out of pwm level range!!!");

	step = (bl_pwm->bl_level_max - bl_pwm->bl_level_min + 2) / 4;
	delta = step / 2;
	step2 = 2 * step;
	step3 = 3 * step;
	step4 = bl_pwm->bl_level_max - step3 - bl_pwm->bl_level_min;

	/*pwm curve mapping*/
	if (levelin < (step + bl_pwm->bl_level_min))
		levelout = bl_do_div(((p1 - p0) * (levelin - bl_pwm->bl_level_min)
			+ delta), step) + p0;
	else if (levelin < (step2 + bl_pwm->bl_level_min))
		levelout = bl_do_div(((p2 - p1) * (levelin - step - bl_pwm->bl_level_min)
			+ delta), step) + p1;
	else if (levelin < (step3 + bl_pwm->bl_level_min))
		levelout = bl_do_div(((p3 - p2) * (levelin - step2 - bl_pwm->bl_level_min)
			+ delta), step) + p2;
	else
		levelout = bl_do_div(((p4 - p3) * (levelin - step3 - bl_pwm->bl_level_min)
			+ (step4 / 2)), step4) + p3;

	if (levelout < bl_pwm->bl_level_min)
		levelout = bl_pwm->bl_level_min;
	else if (levelout > bl_pwm->bl_level_max)
		levelout = bl_pwm->bl_level_max;
...
void bl_pwm_duty_to_pwm_level(struct bl_pwm_config_s *bl_pwm)
{
	...
	bl_pwm->pwm_level =
		bl_do_div(((temp * bl_pwm->pwm_duty) + level_half), bl_pwm->pwm_duty_range);
}
...
void bl_level_to_pwm_level(struct bl_pwm_config_s *bl_pwm)
{
	...
	bl_level = bl_pwm_set_mapping(bl_pwm, bl_level);
	if (bl_level > bl_max) {
		...
		bl_pwm->pwm_level =
			bl_do_div((temp * (bl_level - bl_min)), (bl_max - bl_min)) + pwm_min;
	}
}
...
void bl_pwm_set_duty(struct aml_bl_drv_s *bdrv, struct bl_pwm_config_s *bl_pwm)
{
	...
	if (bdrv->pwm_duty_free) {
		if (bl_pwm->pwm_duty > bl_pwm->pwm_duty_range) {
			BLERR("pwm_duty %d > %d, reset to %d\n",
				bl_pwm->pwm_duty, bl_pwm->pwm_duty_range,
	...
	if (lcd_debug_print_flag & LCD_DBG_PR_BL_ADV) {
		BLPR("pwm port 0x%x: pwm_max=%d, pwm_min=%d, pwm_level=%d\n",
		     bl_pwm->pwm_port,
		      ...
		      struct bl_pwm_config_s *bl_pwm, unsigned int level)
{
	...
	bl_pwm->pwm_duty = (bl_do_div((temp * range * 10), bl_pwm->pwm_cnt) + 5) / 10;

	if (lcd_debug_print_flag & LCD_DBG_PR_BL_ADV) {
		BLPR("pwm port 0x%x: bl_level=%d, bl_level_max=%d, bl_level_min=%d\n",
		     bl_pwm->pwm_port, level, bl_pwm->bl_level_max, bl_pwm->bl_level_min);
		BLPR("duty=%d, pwm max=%d, min=%d, pwm_lvl=%d\n",
			bl_pwm->pwm_duty,
...
void bl_pwm_config_init(struct bl_pwm_config_s *bl_pwm)
{
	...
	case BL_PWM_VS:
		if (bl_pwm->pwm_freq > 4) {
			BLERR("[%d]: pwm_vs wrong freq %d\n",
				bl_pwm->drv_index, bl_pwm->pwm_freq);
	...
	bl_pwm->pwm_max = bl_do_div((temp * bl_pwm->pwm_duty_max), bl_pwm->pwm_duty_range);
	bl_pwm->pwm_min = bl_do_div((temp * bl_pwm->pwm_duty_min), bl_pwm->pwm_duty_range);
