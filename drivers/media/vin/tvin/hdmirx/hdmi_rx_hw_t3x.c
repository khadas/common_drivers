...
static const u32 phy_misc_t3x_21[][3] = {
	...
	{	 /* FRL 3G */
		0x01fcffff, 0x30f7000f, 0x00005a00,
	},
	{	 /* FRL 6G3L */
		0x01fcffff, 0x30f7000f, 0x00005a00,
	},
...
static const u32 phy_dcha_t3x_21[][4] = {
	...
	{	 /* 340~525M */
		0x0320ffff, 0x05ff1a05, 0x51000010, 0x07f06555,
	},
	{	 /* 525~600M */
		0x0320ffff, 0x05ff1a05, 0x51000010, 0x07f06555,
	},
	...
	{	 /* FRL 12G */
		0x2300ffff, 0x05ff1a05, 0x51102000, 0x07f06555,
	},
};
...
static const u32 phy_dchd_t3x_21[][2] = {
		...
		/*  0x130	 0x134 */
	{	 /* 24~35M */
		0x04007ec2, 0x301b3060,
	},
	{	 /* 35~75M */
		0x04007dc2, 0x301b3060,
	},
	{	 /* 75~150M */
		0x04007cc2, 0x30133069,
	},
	{	 /* 140~340M */
		0x040078c2, 0x30133049,
	},
	{	 /* 340~525M */
		0x040074c2, 0x3013304f,
	},
	{	 /* 525~600M */
		0x040074c2, 0x3013304f,
	},
...
void aml_phy_offset_cal_t3x_21(int port)
{
	...
	/* cdr lkdet reset = 0*/
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_CDR,  0x040010c3, port);
...
void rx_21_frl_phy_cfg(u8 port)
{
	...
	hdmirx_wr_bits_amlphy_t3x(T3X_HDMIRX21PHY_MISC2, MSK(3, 0), 0x0, port);
...
void aml_pll_bw_cfg_t3x_21(int f_rate, u8 port)
{
	...
	if (rx_info.aml_phy_21.eq_stg2 <= 0x7)
		hdmirx_wr_bits_amlphy_t3x(T3X_HDMIRX21PHY_DCHD_EQ, BUF_BST,
...
void rx_21_frl_pll_cfg(int f_rate, u8 port)
{
	if (f_rate == FRL_3G_3LANE) {
		//hdmirx_wr_amlphy_t3x(0xc00401e0, 0x5, port);
		...
		rx_pr("T3X_HDMIRX21PLL_CTRL0=0x%x",
			hdmirx_rd_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, port));
	} else if (f_rate == FRL_6G_4LANE) {
		rx_pr("6g 4lane pll_cfg\n");
		//hdmirx_wr_amlphy_t3x(0xc00401e0, 0x5);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL0, 0x0500fa00, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL1, 0x014810e6, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL2, 0x0, port);
		hdmirx_wr_amlphy_t3x(T3X_HDMIRX21PLL_CTRL3, 0x0, port);
...
{
	return (rd_reg_clk_ctl(T3X_CLKCTRL_HDMI_PLL0_CTRL0) >> 31);
}
...
void dump_reg_phy_t3x_21(u8 port)
{
	...
	rx_pr("RX21PHY_DCHA_DFE-0x24=0x%x\n",
...
void aml_phy_power_off_t3x_20(u8 port)
{
	...
	//hdmirx_wr_amlphy_t3x(T3X_HDMIRX_EARCTX_CNTL0, 0x0, port);
	//hdmirx_wr_amlphy_t3x(T3X_HDMIRX_EARCTX_CNTL1, 0x0, port);
	//hdmirx_wr_amlphy_t3x(T3X_HDMIRX_ARC_CNTL, 0x0, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX_PHY_PROD_TEST0, 0x0, port);
	hdmirx_wr_amlphy_t3x(T3X_HDMIRX_PHY_PROD_TEST1, 0x0, port);
}

void aml_phy_power_off_t3x_21(void)
{
	//doto
}
...
void rx_lts_3_err_detect(u8 port)
{
	...
	if (rx[port].var.frl_rate > FRL_RATE_6G_4LANES) {
		...
		rx_pr("bist3:0x%x-0x%x-0x%x\n", bist3_err_cnt_0, bist3_err_cnt_1, bist3_err_cnt_2);
...
void rx_lts_p_syn_detect(u8 frl_rate, u8 port)
{
	...
	hdmirx_wr_cor(H21RXSB_CTRL3_M42H_IVCRX, 0x01, port); //
	//
...
void hdmi_tx_rx_frl_training_main(u8 port)
{
	...
	//rx_pr("[FRL TRAINING] ************** TX_LTS_1_HDMI21_CONFIG************\n");
	...
	//frl_debug
	while (1) {
		data32 = hdmirx_rd_cor(SCDCS_CONFIG1_SCDC_IVCRX, port);
		if ((data32 & 0xf) || cnt++ == frate_cnt) {
			if (log_level & FRL_LOG)
				rx_pr("cnt=%d\n", cnt);
			break;
		}
	...
	if (!hdmirx_get_frl_rate(port)) {
		hdmirx_wr_cor(RX_H21_CTRL_PWD_IVCRX, 0, port);//related to DE status
		rx_pr("frl not support\n");
		return;
	}
	hdmirx_frl_config(port);
	//frl_debug no_timeout
	hdmirx_wr_bits_cor(SCDCS_SRC_TEST_CONFIG_SCDC_IVCRX, _BIT(5), 1, port);
	//rx_pr("[FRL TRAINING] ************** RX_LTS_3_LTP_REQ_SEND_1111************\n");
	//RX_LTS_3_LTP_REQ_SEND_1111(port);
	//st_printf("[FRL TRAINING] ************** TX_LTS_3_POLL_FLT_UPDATE************\n");
	//TX_LTS_3_POLL_FLT_UPDATE(int_ext);
	//st_printf("[FRL TRAINING] ************** TX_LTS_3_READ_LTP_REQ************\n");
	//TX_LTS_3_READ_LTP_REQ(int_ext);
	//st_printf("[FRL TRAINING] ************** TX_FLT_UPDATE_CLEAR************\n");
	//TX_FLT_UPDATE_CLEAR(int_ext);
	//rx_pr("[FRL TRAINING] ************** RX_WAIT_FLT_UPDATE_CLEAR************\n");
	//RX_WAIT_FLT_UPDATE_CLEAR(port);
	//rx_pr("[FRL TRAINING] ************** RX_LTS_3_FRL_CONFIG************\n");
	//RX_LTS_3_FRL_CONFIG(frl_rate, port);//dont need config digital phy
	//rx_pr("[FRL TRAINING] ************** RX_LTS_3_LTP_REQ_SEND_5678************\n");
	//RX_LTS_3_LTP_REQ_SEND_5678(port);
	RX_LTS_3_LTP_REQ_SEND_1111(port);
	//st_printf("[FRL TRAINING] ************** TX_LTS_3_POLL_FLT_UPDATE************\n");
	//TX_LTS_3_POLL_FLT_UPDATE(int_ext);
	//st_printf("[FRL TRAINING] ************** TX_LTS_3_READ_LTP_REQ************\n");
	...
	//rx_pr("[FRL TRAINING] ************** RX_WAIT_FLT_UPDATE_CLEAR************\n");
	RX_WAIT_FLT_UPDATE_CLEAR(port);
	//rx_pr("[FRL TRAINING] ************** aml_phy_init_t3x************\n");
	mdelay(tr_delay0);
	aml_phy_init_t3x_21(port);
	mdelay(tr_delay1);
	//rx_pr("[FRL TRAINING] ************** RX_LTS_3_ERR_DETECT************\n");
	rx_lts_3_err_detect(port);
	//rx_pr("[FRL TRAINING] ************** RX_LTS_3_LTP_REQ_SEND_0000************\n");
	RX_LTS_3_LTP_REQ_SEND_0000(port);
	//st_printf("[FRL TRAINING] ************** TX_LTS_3_POLL_FLT_UPDATE************\n");
	//TX_LTS_3_POLL_FLT_UPDATE(int_ext);
	//st_printf("[FRL TRAINING] ************** TX_LTS_3_FRL_PASS************\n");
	//TX_LTS_3_FRL_PASS(int_ext,frl_rate);
	//rx_pr("[FRL TRAINING] ************** TX_LTS_P_SEND_ONLY_GAP************\n");
	//TX_LTS_P_SEND_ONLY_GAP(frl_rate);
	//st_printf("[FRL TRAINING] ************** TX_FLT_UPDATE_CLEAR************\n");
	//TX_FLT_UPDATE_CLEAR(int_ext);
	//rx_pr("[FRL TRAINING] ************** RX_WAIT_FLT_UPDATE_CLEAR************\n");
	RX_WAIT_FLT_UPDATE_CLEAR(port);
	//rx_pr("[FRL TRAINING] ************** RX_LTS_P_SYN_DETECT************\n");
	rx_lts_p_syn_detect(rx[port].var.frl_rate, port);
	//rx_pr("[FRL TRAINING] ************** RX_LTS_P_FRL_START************\n");
	RX_LTS_P_FRL_START(port);
	//st_printf("[FRL TRAINING] ************** TX_LTS_P_POLL_FRL_START************\n");
	//TX_LTS_P_POLL_FRL_START(int_ext);
}
...
{
	//cancel_work_sync(&frl_train_dwork);
	struct phy_port_data *port_data =
		(struct phy_port_data *)container_of(work, struct phy_port_data, work);
	int port_idx = port_data->port_idx;
...
void hdmirx_frl_config(u8 port)
{
	...
	hdmirx_wr_cor(H21RXSB_CTH2_M42H_IVCRX, 0x01, port);   //clk en th
	/* end */
