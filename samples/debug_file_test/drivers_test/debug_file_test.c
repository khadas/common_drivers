...
static int __init debug_file_test_module_init(void)
{
	...
	write_files = kzalloc(sizeof(*write_files) * WRITE_THREAD_NUM, GFP_KERNEL);
	...
	read_files = kzalloc(sizeof(*read_files) * READ_THREAD_NUM, GFP_KERNEL);
