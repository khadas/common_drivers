...
static void sensor_alloc_integration_time(void *ctx, uint16_t *int_time_S, uint16_t *int_time_M, uint16_t *int_time_L)
{
    ...
    case WDR_MODE_LINEAR: // Normal mode
        if (*int_time_S > p_ctx->vmax - 2)
            *int_time_S = p_ctx->vmax - 2;
        if (*int_time_S < 1)
            *int_time_S = 1;
    ...
    case WDR_MODE_FS_LIN: // DOL3 Frames
        ...
        if (*int_time_M < 2)
            *int_time_M = 2;
        if (*int_time_M > p_ctx->max_M)
            *int_time_M = p_ctx->max_M;
...
static uint16_t sensor_get_id(void *ctx)
{
    ...
    LOG(LOG_CRIT, "%s: success read SUB___290 %x\n", __func__, sensor_id);
    return sensor_id;
}

static void sensor_set_iface(void *ctx, sensor_mode_t *mode)
{
    ...
    memset(&p_ctx->mipi_info, 0, sizeof(p_ctx->mipi_info));
    memset(&info, 0, sizeof(struct am_adap_info));
    p_ctx->mipi_info.lanes = mode->lanes;
    p_ctx->mipi_info.ui_val = 1000 / mode->bps;
    p_ctx->mipi_info.mipi_channel = MIPI1_CHAN;

    if ((1000 % mode->bps) != 0)
        p_ctx->mipi_info.ui_val += 1;
    ...
    if (mode->wdr_mode == WDR_MODE_FS_LIN)
    {
        ...
        info.alt0_path = ADAP1_ALT0_PATH;
        info.path = ADAP0_PATH;
        p_ctx->mipi_info.mipi_channel = MIPI0_CHAN;
        p_ctx->mipi_info.fte1_flag = 1;
    }
    else
    {
        if (p_ctx->dcam_mode)
    ...
    pr_info("Sub IMX290 Dcam:%d, aligh:%d\n", p_ctx->dcam_mode, info.align_width);
    am_mipi_init(&p_ctx->mipi_info);
    am_adap_set_info(&info);
    am_adap_init(info.path);
    am_adap_start(info.path, p_ctx->dcam_mode);
}
...
static void sensor_set_mode(void *ctx, uint8_t mode)
{
    ...
    if (p_ctx->init_sensor++ == 0)
        return;

    setting_num = param->modes_table[mode].num;
    LOG(LOG_CRIT, "%s: sub sensor setting num : %d, wdr mode: %d\n", __func__, setting_num, param->modes_table[mode].wdr_mode);
    ...
    case WDR_MODE_LINEAR:
        ...
        // acamera_sbus_write_u8( p_sbus, 0x3018, 0x65);
        // acamera_sbus_write_u8( p_sbus, 0x3019, 0x08);
        p_ctx->s_fps = 30;
        p_ctx->again_delay = 0;
...
// sensor_load_sequence( p_sbus, p_ctx->seq_width, p_sensor_data, SENSOR_IMX290_SEQUENCE_DEFAULT_WDR_1080P_2DOL );
    ...
    // acamera_sbus_write_u8( p_sbus, 0x3019, 0x6);
    ...
    // sensor_set_iface(p_ctx, &param->modes_table[mode]);
...
static void stop_streaming(void *ctx)
{
    ...
    p_ctx->dcam_mode = 1;
    LOG(LOG_CRIT, "sub sensor %s: \n", __func__);
    acamera_sbus_write_u8(p_sbus, 0x3000, 0x01);

    reset_sensor_bus_counter();

    am_adap_deinit(p_ctx->mipi_info.mipi_channel);
    am_mipi_deinit(p_ctx->mipi_info.mipi_channel);
}
extern void mipi_start(void);
static void start_streaming(void *ctx)
{
    sensor_context_t *p_ctx = ctx;
    acamera_sbus_ptr_t p_sbus = &p_ctx->sbus;
    sensor_param_t *param = &p_ctx->param;
    LOG(LOG_CRIT, "sub imx290 start streaming");
    sensor_set_iface(p_ctx, &param->modes_table[p_ctx->sen_mode]);
...
static void sensor_test_pattern(void *ctx, uint8_t mode)
{
    ...
    if (mode == 0xff)
    {
        LOG(LOG_CRIT, "Donot skip initial sensor: %d", p_ctx->init_sensor);
        p_ctx->init_sensor += 1;
        return;
    }
...
void sensor_deinit_imx290_sub(void *ctx)
{
    sensor_context_t *t_ctx = ctx;
    reset_sensor_bus_counter();
...
//--------------------Initialization------------------------------------------------------------
void sensor_init_imx290_sub(void **ctx, sensor_control_t *ctrl, void *sbp)
{
...
/*
    ...
    write2_reg(0x9090, 0xfe007cd4);

    write2_reg(0x11300300, 0xfe000428);
*/
#if PLATFORM_C305X
