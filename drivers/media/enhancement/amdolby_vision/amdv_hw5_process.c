					      ...
					      bool drop_flag)
{
	...
	if (is_aml_tvmode() && vf &&
		...
		if (((debug_dolby & 1) || v_inst_info->frame_count == 0) && toggle_mode == 1)
			pr_info
			("DV:[%d,%lld,%d,%s,%d,%d]\n",
			 v_inst_info->frame_count, vf->pts_us64, src_bdp,
			 (src_format == FORMAT_HDR10) ? "HDR10" :
...
int amdv_wait_metadata_hw5(struct vframe_s *vf)
{
	...
	if (!top2_v_info.amdv_wait_init && !top2_info.top2_on) {
		...
		u32 display_size)
{
	...
	if (vf && (debug_dolby & 0x8))
		pr_dv_dbg("top1 proc: vf %p(index %d),mode %d,top2_on %d,flag %x\n",
				  vf, vf->omx_index, dolby_vision_mode,
				  top2_info.top2_on, dolby_vision_flags);
	reset_flag =
		(amdv_reset & 1) &&
		(!top2_info.top2_on) &&
