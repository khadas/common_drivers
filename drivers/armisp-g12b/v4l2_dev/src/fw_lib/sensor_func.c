...
void sensor_hw_init( sensor_fsm_ptr_t p_fsm )
{
    ...
    do_gettimeofday(&txs);
#else
    struct timespec64 txs, txe;
    ktime_get_real_ts64(&txs);
#endif
    ...
    do_gettimeofday(&txe);
#else
    ktime_get_real_ts64(&txe);
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 0))
    LOG( LOG_INFO, "sensor_hw_init:%d", (txe.tv_sec*1000000+txe.tv_usec) -  (txs.tv_sec*1000000+txs.tv_usec));
#else
    LOG(LOG_INFO, "sensor_hw_init:%d ns", (txe.tv_sec * 1000000000 + txe.tv_nsec) - (txs.tv_sec * 1000000000 + txs.tv_nsec));
#endif
