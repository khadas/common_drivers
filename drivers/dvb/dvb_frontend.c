...
void amlogic_dvb_frontend_add_event(struct dvb_frontend *fe,
			    enum fe_status status)
#else
static void amlogic_dvb_frontend_add_event(struct dvb_frontend *fe,
				   enum fe_status status)
#endif
{
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
...
#ifdef CONFIG_AMLOGIC_DVB_COMPAT
EXPORT_SYMBOL(amlogic_dvb_frontend_add_event);
#endif
...
void amlogic_dvb_frontend_reinitialise(struct dvb_frontend *fe)
{
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
...
EXPORT_SYMBOL(amlogic_dvb_frontend_reinitialise);
...
static void dvb_frontend_swzigzag(struct dvb_frontend *fe)
{
	...
	if (fepriv->state & FESTATE_RETUNE) {
		...
		if (s != fepriv->status) {
			amlogic_dvb_frontend_add_event(fe, s);
			fepriv->status = s;
		}
	}
...
restart:
		...
		if (fe->ops.get_frontend_algo) {
			...
			case DVBFE_ALGO_CUSTOM:
				...
				fe->ops.read_status(fe, &s);
				if (s != fepriv->status) {
					amlogic_dvb_frontend_add_event(fe, s); /* update event list */
					fepriv->status = s;
					if (!(s & FE_HAS_LOCK)) {
						fepriv->delay = HZ / 10;
						fepriv->algo_status |= DVBFE_ALGO_SEARCH_AGAIN;
 ...
 */
void amlogic_dvb_frontend_sleep_until(ktime_t *waketime, u32 add_usec)
{
	s32 delta;
...
EXPORT_SYMBOL(amlogic_dvb_frontend_sleep_until);
...
static int dtv_set_frontend(struct dvb_frontend *fe)
{
	...
	amlogic_dvb_frontend_add_event(fe, 0);
	dvb_frontend_wakeup(fe);
	fepriv->status = 0;
...
int amlogic_dvb_frontend_suspend(struct dvb_frontend *fe)
{
	int ret = 0;
...
EXPORT_SYMBOL(amlogic_dvb_frontend_suspend);

int amlogic_dvb_frontend_resume(struct dvb_frontend *fe)
{
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
	int ret = 0;
...
EXPORT_SYMBOL(amlogic_dvb_frontend_resume);

int amlogic_dvb_register_frontend(struct dvb_adapter *dvb,
			  struct dvb_frontend *fe)
{
...
EXPORT_SYMBOL(amlogic_dvb_register_frontend);

int amlogic_dvb_unregister_frontend(struct dvb_frontend *fe)
{
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
...
EXPORT_SYMBOL(amlogic_dvb_unregister_frontend);
...
void amlogic_dvb_frontend_detach(struct dvb_frontend *fe)
{
...
EXPORT_SYMBOL(amlogic_dvb_frontend_detach);
