...
static void sysfs_create(struct dm_target *ti)
{
	int ret;

	if (ti && ti->private && ti->table) {
		struct bow_context *bc = ti->private;
...
static struct dm_target *dm_table_find_bow_target(struct dm_table *t)
{
	int i;
	...
	for (i = 0; i < t->num_targets; i++) {
		...
		pr_info("targets type name %s\n", ti->type->name);

		if (!strcmp(ti->type->name, "bow"))
			return ti;
	}
...
static int _bow_sysfs_create(struct dm_table *table,
			  struct dm_ioctl *param, size_t param_size)
{
	int r;
	unsigned int i = 0;
	struct dm_target_spec *spec = (struct dm_target_spec *)param;
	u32 next = param->data_start;
	...
	for (i = 0; i < param->target_count; i++) {
		...
		if (!strcmp(spec->target_type, "bow")) {
			struct dm_target *ti = dm_table_find_bow_target(table);
...
int bow_sysfs_create(struct dm_table *table,
			  struct dm_ioctl *param, size_t param_size)
{
	return _bow_sysfs_create(table, param, param_size);
}
				   ...
				   bool record_checksum)
{
	...
	backup_bi.bi_size = min_t(u64,
				  range_size(backup_br),
				 (u64)bi_iter->bi_size);
