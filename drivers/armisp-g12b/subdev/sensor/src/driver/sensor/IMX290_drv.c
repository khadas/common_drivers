...
static uint16_t sensor_get_id(void *ctx)
{
    ...
    LOG(LOG_CRIT, "%s: success read 290 %x\n", __func__, sensor_id);
    return sensor_id;
}
...
static void sensor_set_iface(void *ctx, sensor_mode_t *mode)
{
    ...
    LOG(LOG_CRIT, "%s: p_ctx->dcam_mode : %d\n", __func__, p_ctx->dcam_mode);
...
// sensor_load_sequence( p_sbus, p_ctx->seq_width, p_sensor_data, SENSOR_IMX290_SEQUENCE_DEFAULT_WDR_1080P_2DOL );
    ...
    // acamera_sbus_write_u8( p_sbus, 0x3019, 0x1f);
    ...
    LOG(LOG_CRIT, "Output resolution from sensor: %dx%d", param->active.width, param->active.height); // LOG_NOTICE Causes errors in some projects
}
...
static void stop_streaming(void *ctx)
{
    ...
    p_ctx->dcam_mode = 1;
    LOG(LOG_CRIT, "sensor %s: \n", __func__);
    acamera_sbus_write_u8(p_sbus, 0x3000, 0x01);

    reset_sensor_bus_counter();
    LOG(LOG_CRIT, "sensor %s: mipi_info.mipi_channel: %d \n", __func__, p_ctx->mipi_info.mipi_channel);
...
static void start_streaming(void *ctx)
{
    ...
    acamera_sbus_ptr_t p_sbus = &p_ctx->sbus;
    sensor_param_t *param = &p_ctx->param;
    LOG(LOG_CRIT, "imx290 start streaming");
...
{
    LOG(LOG_CRIT, "%s: get dcam: %d", __func__, mode);
    sensor_context_t *p_ctx = ctx;
...
*/
#if NEED_CONFIG_BSP
    ret = gp_pl_am_enable(sensor_bp, "g12a_24m", 37125000); // g12a_24m
    if (ret < 0)
