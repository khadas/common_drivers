...
//
//Values:
//  {DMA_READER_OUT_FR} - output is full resolution pipe
...
{
    uint16_t dpc_threshold = 0;
    uint16_t dpc_slope = 0;

    switch ( direction ) {
    case COMMAND_SET:
        dpc_threshold = ( value >> 16 ) & 0xFFF;
        dpc_slope = value & 0xFFF;
        acamera_isp_raw_frontend_dp_threshold_write(ACAMERA_MGR2CTX_PTR( instance )->settings.isp_base, dpc_threshold);
        acamera_isp_raw_frontend_dp_slope_write(ACAMERA_MGR2CTX_PTR( instance )->settings.isp_base, dpc_slope);

        return SUCCESS;
    case COMMAND_GET:
        dpc_threshold = acamera_isp_raw_frontend_dp_threshold_read(ACAMERA_MGR2CTX_PTR( instance )->settings.isp_base);
        dpc_slope = acamera_isp_raw_frontend_dp_slope_read(ACAMERA_MGR2CTX_PTR( instance )->settings.isp_base);
        *ret_value = (dpc_threshold << 16) | dpc_slope;
...
static uint8_t acamera_calibration_update( acamera_fsm_mgr_t *instance, uint32_t id, uint8_t direction, void *data, uint32_t data_size, uint32_t *ret_value )
{
    ...
    *ret_value = 0;
    if ( id < CALIBRATION_TOTAL_SIZE && data != NULL ) {
        // we require precise size of source and destination
