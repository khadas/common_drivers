...
struct SensorConversion ConversionTable[] = {
    ...
    {SENSOR_INIT_SUBDEV_FUNCTIONS_IMX290SUB, SENSOR_DEINIT_SUBDEV_FUNCTIONS_IMX290SUB, "imx290"},
};
...
static long camera_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    {
        LOG(LOG_CRIT, "Failed to process camera_ioctl. Sensor is not initialized yet. camera_init must be called before");
        rc = -1;
        return rc;
    }
    ...
    case SOC_SENSOR_STREAMING_ON:
        LOG(LOG_CRIT, "%s: sensor start streaming. \n", __func__);
        ctx->camera_control.start_streaming(ctx->camera_context);
        break;
    case SOC_SENSOR_STREAMING_OFF:
        LOG(LOG_CRIT, "%s: sensor stop %d streaming. \n", __func__, ARGS_TO_PTR(arg)->ctx_num);
