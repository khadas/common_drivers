...
#undef pr_fmt
#define pr_fmt(fmt) "t3x_audio_clocks: " fmt

#include <dt-bindings/clock/amlogic,t3x-audio-clk.h>
...
static struct clk_gate *t3x_audio_clk_gates[] = {
	&audio_ddr_arb,
	&audio_pdm,
	&audio_tdmina,
	&audio_tdminb,
...
static struct clk_hw *t3x_audio_clk_hws[] = {
...
static int t3x_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(t3x_audio_clk_gates) != MCLK_BASE) {
	...
	for (clkid = 0; clkid < MCLK_BASE; clkid++) {
		unsigned long offset =
			(unsigned long)t3x_audio_clk_gates[clkid]->reg;
		t3x_audio_clk_gates[clkid]->reg =
			(void __iomem *)((unsigned long)iobase + offset);
		clks[clkid] = clk_register(NULL, t3x_audio_clk_hws[clkid]);
...
/* audio resample_c */
CLOCK_COM_MUX(resample_c,
	AUD_ADDR_OFFSET(EE_AUDIO_CLK_RESAMPLEC_CTRL), 0xf, 24);
CLOCK_COM_DIV(resample_c, AUD_ADDR_OFFSET(EE_AUDIO_CLK_RESAMPLEC_CTRL), 0, 8);
CLOCK_COM_GATE(resample_c, AUD_ADDR_OFFSET(EE_AUDIO_CLK_RESAMPLEC_CTRL), 31);
...
static int t3x_clks_init(struct clk **clks, void __iomem *iobase)
{
	...
	IOMAP_COM_CLK(resample_c, iobase);
	clks[CLKID_AUDIO_RESAMPLE_C] = REGISTER_AUDIOCLK_COM(resample_c);
	WARN_ON(IS_ERR_OR_NULL(clks[CLKID_AUDIO_RESAMPLE_C]));
...
struct audio_clk_init t3x_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = t3x_clk_gates_init,
	.clks      = t3x_clks_init,
};
